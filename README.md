...
index.html               # Standalone prototype scene (non‑Nuxt)
```

Notes:
- Do not edit files in `.nuxt/`; they are generated by Nuxt.
- Static assets placed under `public/` are available at `/assets/...`.
- `index.html` is a legacy/standalone prototype that can be opened directly in a browser or hosted as a plain static page. The main app lives under `app/` via Nuxt.
...
- Mobile zoom is disabled to prioritize the intended “spotlight” experience. If you need to re‑enable zoom for accessibility, edit the viewport meta in `nuxt.config.ts` and `index.html`, and remove `touch-action: manipulation` in CSS.
...
- Standalone prototype:
  - Serve the root `index.html` with its assets (no build step needed). This is best for quick previews and kiosks but lacks Nuxt features.
